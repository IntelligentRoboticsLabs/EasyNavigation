...
int main(int argc, char ** argv)
{
  ...
  auto planner_node = easynav_planner::PlannerNode::make_shared();

  exe_nort.add_node(planner_node->get_node_base_interface());
  exe_rt.add_callback_group(planner_node->get_real_time_cbg(),
    planner_node->get_node_base_interface());

  planner_node->trigger_transition(lifecycle_msgs::msg::Transition::TRANSITION_CONFIGURE);
  if (planner_node->get_current_state().id() !=
    lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE)
  {
    RCLCPP_ERROR(planner_node->get_logger(), "Unable to configure EasyNav");
    rclcpp::shutdown();
    return 1;
  }
  ...
  {
    RCLCPP_ERROR(planner_node->get_logger(), "Unable to activate EasyNav");
    rclcpp::shutdown();
    return 1;
  }
  ...
  auto rt_thread = std::thread(
    [&]() {
      ...
      if (sched_setscheduler(0, SCHED_FIFO, &sch) == -1) {
        RCLCPP_ERROR(
          planner_node->get_logger(),
          "Failed to tet EasyNav to execute in Real Time.");
        RCLCPP_ERROR(planner_node->get_logger(), "set your system to have permissions.");
