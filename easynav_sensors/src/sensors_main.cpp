...
int main(int argc, char ** argv)
{
  ...
  exe_nort.add_node(sensors_node->get_node_base_interface());
  exe_rt.add_callback_group(sensors_node->get_real_time_cbg(),
    sensors_node->get_node_base_interface());

  sensors_node->trigger_transition(lifecycle_msgs::msg::Transition::TRANSITION_CONFIGURE);
  if (sensors_node->get_current_state().id() !=
    lifecycle_msgs::msg::State::PRIMARY_STATE_INACTIVE)
  {
    RCLCPP_ERROR(sensors_node->get_logger(), "Unable to configure EasyNav");
    rclcpp::shutdown();
    return 1;
  }
  ...
  {
    RCLCPP_ERROR(sensors_node->get_logger(), "Unable to activate EasyNav");
    rclcpp::shutdown();
    return 1;
  }
  ...
  auto rt_thread = std::thread(
    [&]() {
      ...
      if (sched_setscheduler(0, SCHED_FIFO, &sch) == -1) {
        RCLCPP_ERROR(
          sensors_node->get_logger(),
          "Failed to tet EasyNav to execute in Real Time.");
        RCLCPP_ERROR(sensors_node->get_logger(), "set your system to have permissions.");
